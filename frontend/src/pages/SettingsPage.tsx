...
export default () => {
  const { dispatch, cookie, removeCookie, setCookie } = useAuthContext();
  ...
  const handleChangePassword = async () => {
    ...
    const accessToken = cookie.userCred;
    const res = await axios
      .post(URL_USER_CHANGE_PW_SVC, { password: password, newPassword: newPassword1, accessToken: accessToken })
      .catch((err) => {
        if (err.response.status === STATUS_CODE_FORBIDDEN) {
